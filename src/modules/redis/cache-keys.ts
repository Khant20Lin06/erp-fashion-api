...
export const CacheKeys = {
  ...
  departments: (companyId: string): string =>
    `erp:cache:hr:departments:${companyId}`,
  designations: (companyId: string): string =>
    `erp:cache:hr:designations:${companyId}`,
  settingValue: (scopeType: string, scopeId: string, key: string): string =>
    `erp:cache:settings:${scopeType.toLowerCase()}:${scopeId}:${key}`,
  /**
...
/**
 * Default TTLs (seconds) for each cached resource - short enough that a
 * stale write is invisible within one cache lifetime, long enough to
 * actually reduce MySQL load for read-heavy master data.
 */
export const CacheTtl = {
  PAYMENT_METHODS_SECONDS: 300,
  COMPANY_SETTINGS_SECONDS: 300,
  HR_MASTER_SECONDS: 300,
  SETTINGS_SECONDS: 300,
  /**
   * Phase 22 - short TTL (locked spec: 60-300s) so dashboard
   * financial-summary staleness is always tightly bounded.
   */

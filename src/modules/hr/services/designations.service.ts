import { Injectable } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Brackets, Repository } from 'typeorm';
import { Designation } from '../entities/designation.entity';
import {
  CreateDesignationDto,
  ListDesignationsDto,
  UpdateDesignationDto,
} from '../dto/designations.dto';
import { CompaniesService } from '../../organization/services/companies.service';
import { AppException } from '../../../core/errors/app.exception';
import { ErrorCode } from '../../../core/errors/error-codes';
import {
  DEFAULT_LIMIT,
  DEFAULT_PAGE,
} from '../../../shared/dto/pagination.dto';
import { resolveSortField } from '../../../shared/dto/resolve-sort-field';
import { DesignationStatus } from '../entities/designation-status.enum';
import { EmployeeAssignment } from '../entities/employee-assignment.entity';

const SORTABLE_FIELDS = ['createdAt', 'name', 'code', 'status'] as const;

@Injectable()
export class DesignationsService {
  constructor(
    @InjectRepository(Designation)
    private readonly designationRepository: Repository<Designation>,
    @InjectRepository(EmployeeAssignment)
    private readonly assignmentRepository: Repository<EmployeeAssignment>,
    private readonly companiesService: CompaniesService,
  ) {}

  async findAll(
    companyId: string,
    query: ListDesignationsDto,
  ): Promise<{
    data: Designation[];
    meta: { page: number; limit: number; total: number };
  }> {
    const page = query.page ?? DEFAULT_PAGE;
    const limit = query.limit ?? DEFAULT_LIMIT;
    const sortField = resolveSortField(query.sort, SORTABLE_FIELDS, 'name');

    const qb = this.designationRepository
      .createQueryBuilder('designation')
      .where('designation.companyId = :companyId', { companyId });

    if (query.status) {
      qb.andWhere('designation.status = :status', { status: query.status });
    }
    if (query.search) {
      qb.andWhere(
        new Brackets((sub) => {
          sub
            .where('designation.name LIKE :search', {
              search: `%${query.search}%`,
            })
            .orWhere('designation.code LIKE :search', {
              search: `%${query.search}%`,
            });
        }),
      );
    }

    qb.orderBy(`designation.${sortField}`, query.order ?? 'ASC')
      .skip((page - 1) * limit)
      .take(limit);

    const [data, total] = await qb.getManyAndCount();
    return { data, meta: { page, limit, total } };
  }

  async findByIdInCompany(id: string, companyId: string): Promise<Designation> {
    const entity = await this.designationRepository.findOne({
      where: { id, companyId },
    });
    if (!entity) {
      throw new AppException(ErrorCode.NotFound, 'Designation not found');
    }
    return entity;
  }

  async create(
    companyId: string,
    dto: CreateDesignationDto,
  ): Promise<Designation> {
    const company = await this.companiesService.findActiveByIdOrNull(companyId);
    if (!company) {
      throw new AppException(
        ErrorCode.ValidationError,
        'companyId does not reference an active company',
      );
    }

    await this.assertUnique(companyId, dto.name, dto.code);

    const entity = this.designationRepository.create({
      companyId,
      name: dto.name,
      code: dto.code ?? null,
      description: dto.description ?? null,
      status: DesignationStatus.Active,
    });

    return this.designationRepository.save(entity);
  }

  async update(
    id: string,
    companyId: string,
    dto: UpdateDesignationDto,
  ): Promise<Designation> {
    const entity = await this.findByIdInCompany(id, companyId);
    const nextName = dto.name ?? entity.name;
    const nextCode =
      dto.code === undefined ? entity.code : dto.code?.trim() || null;
    await this.assertUnique(companyId, nextName, nextCode, id);

    if (dto.name !== undefined) entity.name = dto.name;
    if (dto.code !== undefined) entity.code = dto.code?.trim() || null;
    if (dto.description !== undefined) {
      entity.description = dto.description?.trim() || null;
    }
    return this.designationRepository.save(entity);
  }

  async activate(id: string, companyId: string): Promise<Designation> {
    const entity = await this.findByIdInCompany(id, companyId);
    entity.status = DesignationStatus.Active;
    return this.designationRepository.save(entity);
  }

  async deactivate(id: string, companyId: string): Promise<Designation> {
    const entity = await this.findByIdInCompany(id, companyId);
    entity.status = DesignationStatus.Inactive;
    return this.designationRepository.save(entity);
  }

  async remove(id: string, companyId: string): Promise<void> {
    const entity = await this.findByIdInCompany(id, companyId);
    const assignmentCount = await this.assignmentRepository.count({
      where: { designationId: id },
    });
    if (assignmentCount > 0) {
      throw new AppException(
        ErrorCode.Conflict,
        'Designation is referenced by employee assignments and cannot be deleted',
      );
    }
    await this.designationRepository.softRemove(entity);
  }

  private async assertUnique(
    companyId: string,
    name: string,
    code?: string | null,
    ignoreId?: string,
  ): Promise<void> {
    const qb = this.designationRepository
      .createQueryBuilder('designation')
      .where('designation.companyId = :companyId', { companyId })
      .andWhere('(designation.name = :name OR designation.code = :code)', {
        name,
        code: code ?? null,
      });

    if (ignoreId) {
      qb.andWhere('designation.id != :ignoreId', { ignoreId });
    }

    const existing = await qb.getOne();
    if (existing) {
      throw new AppException(
        ErrorCode.Conflict,
        'Designation name or code already exists for this company',
      );
    }
  }
}
